...
public class ThirdPersonCameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector3 inputDirecetion 
            = _orientationTransform.forward * verticalInput + _orientationTransform.right * horizontalInput;

        if (inputDirecetion != Vector3.zero)
        {
            _playerVisualTransform.forward
            = Vector3.Slerp(_playerVisualTransform.forward, inputDirecetion.normalized, Time.deltaTime * _rotationSpeed);
        }
